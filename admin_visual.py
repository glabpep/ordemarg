...
# --- ESTILIZAÇÃO PARA REMOVER BORDAS DO STREAMLIT ---
st.markdown("""
    <style>
...
# --- 1. SEU CÓDIGO HTML (PARTES 1 E 2 QUE VOCÊ ENVIOU) ---
# Dica: Coloquei os dados do Excel dentro da variável JS 'PRODUTOS' automaticamente
def obter_html_vendas():
    ...
    if os.path.exists(caminho_dados):
        ...
        for idx, row in df.iterrows():
            lista_prods.append({
                ...
                "info": "Informação técnica..." # Aqui você pode expandir com seu dicionário
            })
...
if __name__ == "__main__":
    ...
    <body>
        <div class="container">
            <img src="https://github.com/glabpep/ordem/blob/main/1.png?raw=true" style="width:100%; max-width:300px; display:block; margin:auto;">
            <h2>Estoque Atualizado e Pedidos Online</h2>
        ...
        <script>
            const PRODUTOS = {produtos_js};
            // Cole aqui toda a lógica de JS que você enviou (calcularFrete, enviarPedido, etc)
...
# Botão discreto na lateral para funcionários
with st.sidebar:
    st.image("1.png", width=100)
    if st.button("⚙️ Área do Funcionário"):
...
# A PRIMEIRA PÁGINA (LOJA)
if st.session_state.view == "loja":
    html_da_loja = obter_html_vendas()
    components.html(html_da_loja, height=1200, scrolling=True)
...
# PAINEL ADMIN (ESTOQUE)
    ...
    caminho_dados = os.path.join(os.path.dirname(__file__), 'stock_0202 - NOVA.xlsx')
    if os.path.exists(caminho_dados):
        df = pd.read_excel(caminho_dados)
